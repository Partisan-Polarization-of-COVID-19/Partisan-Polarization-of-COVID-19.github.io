function init_table(options) {
  ...
  $.when($.get(csv_path)).then(
    (data) => {
      ...
      load_table(options, csv_data);
    },
    () => {
      var csv_data = [["Sentiment", "User", "Tweet Text"]];
      load_table(options, csv_data);
    }
  );
}

function load_table(options, csv_data) {
  options = options || {};
  ...
  $("#" + el).html(
    "<table class='table table-striped table-condensed' id='my-table'></table>"
  );
